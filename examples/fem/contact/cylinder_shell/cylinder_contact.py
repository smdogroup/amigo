...
bc_map = {
    "pinned_bottom": {
        ...
        "target": ["LINE2"],  # line along the z direction
    },
}
...
class GapConstraint(am.Component):
    ...
    def compute(self):
        x = self.data["x_coord"]
        u = self.inputs["u"]
        x_floor = self.data["x_floor"]
        self.constraints["gap"] = (x + u) - x_floor


all_nodes = mesh.get_nodes_in_domain("SURFACE1")
bc_nodes = set(mesh.get_nodes_in_domain("LINE2"))
contact_nodes = [i for i in all_nodes if i not in bc_nodes]

model.add_component("contact", len(contact_nodes), GapConstraint())
model.link("fem.soln.u", "contact.u", src_indices=contact_nodes)
model.link("fem.geo.x", "contact.x_coord", src_indices=contact_nodes)
