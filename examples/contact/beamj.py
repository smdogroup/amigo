...
class beam_element(am.Component):
    ...
    def compute(self):
        ...
        # Create local beam element stiffness matrix using actual element length
        A = 1.0
        I = 1.0
        a1 = E * A / L
        a2 = E * I / (L**3.0)
        ...
        Ke = [
            ...
            [0, 6 * L * a2, 4 * L2 * a2, 0, -6 * L * a2, 2 * L2 * a2],
            [-a1, 0, 0, a1, 0, 0],
            [0, -12 * a2, -6 * L * a2, 0, 12 * a2, -6 * L * a2],
            [0, 6 * L * a2, 2 * L2 * a2, 0, -6 * L * a2, 4 * L2 * a2],
        ]
...
class AppliedLoad(am.Component):
    ...
    def __init__(self):
        ...
        self.add_constant("Fv", value=-1.0e6)  # Force in v direction
        self.add_constant("Mt", value=0.0)  # Moment (if any)
        self.add_objective("work")
        return
...
class GapConstraint(am.Component):
    ...
    def compute(self):
        ...
        v = self.inputs["v"]
        y_floor = -0.45
        self.constraints["gap"] = y + v - y_floor
...
model.link("src.u", "bcs.u", src_indices=bc_nodes)
model.link("src.v", "bcs.v", src_indices=bc_nodes)
model.link("src.t", "bcs.t", src_indices=bc_nodes)
...
model.link("src.y_coord", "contact.y_coord", src_indices=non_bc_nodes)
model.link("src.v", "contact.v", src_indices=non_bc_nodes)
...
# model.link("src.u", "comp.u", src_indices=[0, 2])
# model.link("src.v", "comp.v", src_indices=[0, 2])
# model.link("src.t", "comp.t", src_indices=[0, 2])
...
#     y_floor = -0.45
#     lower_bound_nodal = y_floor - y_coord[i]
#     lower[f"src.v[{i}]"] = lower_bound_nodal

#     # penetration = None
#     # if lower_bound_nodal > 0:
#     #     penetration = ', penetrates'
#     # print(lower_bound_nodal, penetration)
...
def plot_disp(ax, x, u=None, scale=1.0):
    """
    ...
    for index in range(nelems):
        i = conn[index, 0]
        j = conn[index, 1]
        ax.plot([x[i, 0], x[j, 0]], [x[i, 1], x[j, 1]], "-ko")
