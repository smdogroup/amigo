"""Quick test: compare Riccati solve vs PARDISO on spaceshuttle."""
...
# ---- Inline spaceshuttle model (avoid importing module-level code) ----
num_intervals = 100
tf_fixed = 2000.0
N = num_intervals + 1
...
class ShuttleCollocation(am.Component):
    def __init__(self, sc, dt):
        ...
        self.add_constant("mu", value=mu_g)
    ...
    def compute(self):
        q1, q2, dt = self.inputs["q1"], self.inputs["q2"], self.constants["dt"]
...
model.add_component("ic", 1, InitialConditions(scaling))
model.add_component("fc", 1, FinalConditions(scaling))
model.add_component("heat", N, HeatingConstraint(scaling))
...
print(f"Num variables: {model.num_variables}, constraints: {model.num_constraints}")
...
x["shuttle.q[:, 1]"] = np.zeros(N)
x["shuttle.q[:, 2]"] = np.zeros(N)
x["shuttle.q[:, 3]"] = v_init / scaling["v"]
x["shuttle.q[:, 4]"] = gamma_init
...
# Optimize with Riccati comparison ON + force BISC
...
data = opt.optimize(
    {
        ...
        "block_psd_convexification": True,
        "nonconvex_constraints": ["colloc.res", "heat.res"],
        "quality_function_l_max": 10,
        "riccati_solve_compare": True,
        "block_psd_force_bisc": True,
    }
)
