...
# Also enforce 1 - n*kappa > 0 to prevent sdot singularity.
# At each node: n < 0.9/kappa (if kappa>0), n > 0.9/kappa (if kappa<0).
kappa_margin = 0.9  # keep 1-n*kappa >= 0.1
n_upper_1 = w_left_1 / scaling["n"]
n_lower_1 = -w_right_1 / scaling["n"]
n_upper_2 = w_left_2 / scaling["n"]
n_lower_2 = -w_right_2 / scaling["n"]
for i in range(num_intervals):
    k1 = kappa_nodes[i]
    k2 = kappa_nodes[i + 1]
    if abs(k1) > 1e-6:
        n_sing_1 = kappa_margin / (k1 * scaling["n"])
        if k1 > 0:
            n_upper_1[i] = min(n_upper_1[i], n_sing_1)
        else:
            n_lower_1[i] = max(n_lower_1[i], n_sing_1)
    if abs(k2) > 1e-6:
        n_sing_2 = kappa_margin / (k2 * scaling["n"])
        if k2 > 0:
            n_upper_2[i] = min(n_upper_2[i], n_sing_2)
        else:
            n_lower_2[i] = max(n_lower_2[i], n_sing_2)
lower["colloc.q1[:, 1]"] = n_lower_1
upper["colloc.q1[:, 1]"] = n_upper_1
lower["colloc.q2[:, 1]"] = n_lower_2
upper["colloc.q2[:, 1]"] = n_upper_2

# Heading angle alpha: keep |alpha| < pi/3 to prevent cos(alpha-lam)->0 singularity in sdot
alpha_max = (np.pi / 3) / scaling["alpha"]
for end in ["1", "2"]:
    lower[f"colloc.q{end}[:, 3]"] = -alpha_max
    upper[f"colloc.q{end}[:, 3]"] = alpha_max

# Slip angle lam: keep |lam| < 0.25 rad (beyond tire capability)
lam_max = 0.25 / scaling["lam"]
for end in ["1", "2"]:
    lower[f"colloc.q{end}[:, 4]"] = -lam_max
    upper[f"colloc.q{end}[:, 4]"] = lam_max

# Steering angle delta: keep |delta| < 0.5 rad (~29 deg)
...
# Per-node n bounds for nc: track width + singularity prevention (same as colloc)
nc_n_lower = -track.w_right / scaling["n"]
nc_n_upper = track.w_left / scaling["n"]
for i in range(num_nodes):
    ki = kappa_nodes[i]
    if abs(ki) > 1e-6:
        n_sing = kappa_margin / (ki * scaling["n"])
        if ki > 0:
            nc_n_upper[i] = min(nc_n_upper[i], n_sing)
        else:
            nc_n_lower[i] = max(nc_n_lower[i], n_sing)
lower["nc.q[:, 1]"] = nc_n_lower
upper["nc.q[:, 1]"] = nc_n_upper

# Same alpha, lam, delta bounds on nc (node constraint) variables
...
try:
    ...
    # Convergence history
    nrms = [it["residual"] for it in opt_data["iterations"]]
