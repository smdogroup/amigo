...
class Compliance(am.Component):
    ...
    def compute_output(self):
        ...
        Fv = self.constants["Fv"]
        Mt = self.constants["Mt"]
        self.outputs["c"] = Fv * v  + Mt * t # dot(Fv, v), but only for one nodal v displ.
...
indeps.add_output("h", shape=nelems, val=5*np.linspace(om_h[0],om_h[-1],len(om_h))) # substitute openmdao results
...
for i in range(n):
    ...
    plt.scatter(i, np.mean(data['fea']['c','h']['J_fwd']))
