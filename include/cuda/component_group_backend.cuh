...
class CudaGroupBackend {
 public:
  ...
  T lagrangian_kernel(const IndexLayout<ndata>& data_layout,
    ...
    // gradient_kernel_atomic<<<grid, block>>>(num_elements, data_indices, data_values, 
  ...
  void add_hessian_product_kernel(const IndexLayout<ndata>& data_layout,
    ...
    hessian_product_kernel_atomic<<<grid, block>>>(num_elements, data_indices, data_values, 
        vec_indices, vec_values, res_values);
  }
 ...
 private:
  static AMIGO_KERNEL void gradient_kernel_atomic(int num_elements,
    const int *data_indices,
    const int *vec_indices, 
    const T *data_values, 
    const T *vec_values,
    ...
    Input input, grad;
    Data data;
    IndexLayout<ndata>::get_values_device(index, dat_indices, data_values, data);
    ...
    IndexLayout<ndomp>::add_values_atomic(index, vec_indices, grad, grad_values);
  }

  template <class Component, class... Remain>
  static AMIGO_DEVICE void add_gradient(const Data& data, const Input& input, Input& grad) {  
  ...
  static AMIGO_KERNEL void hessian_product_kernel_atomic(int num_elements,
    const int *data_indices,
    const int *vec_indices, 
    const T *data_values, 
    const T *vec_values,
  ...
  template <class Component, class... Remain>
  static AMIGO_DEVICE void add_hessian_product(const Data& data, const Input& input,
